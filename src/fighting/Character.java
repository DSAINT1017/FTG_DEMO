...
public class Character {
  ...
  /*耐力*/
	 ...
	 * 一個類構造函數，它使用作為參數傳遞的此類實例的信息創建一個新實例。
	 * @param character
	...
	public Character(Character character) {
		...
		this.endurance = character.getEndurance();
	...
	public Character(CharacterData characterData, ArrayList<Motion> motionList) {
		...
		this.endurance = characterData.getEndurance();
	}

	/**
	 * 設定ファイル(gSetting.txt)の情報を用いてキャラクターの画像サイズを初期化する．
	 * 使用設置文件 (gSetting.txt) 中的信息初始化字符的圖像大小。
	 * @param characterName
	 *            the character name
	 * @param playerNumber
	 ...
	 * 作為參數傳遞的動作信息在 Character 實例中設置為正在執行的動作。
	 * @param executeAction
	 *            次に実行するアクション
	 * @param resetFlag
	...
	public void costEndurance(){
		//System.out.print("costEndurance");
		//System.out.print('\n');
	}
